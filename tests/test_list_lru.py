...
from drgn import IntegerLike, Object, Path, Program, cast
...
from drgn_tools.list_lru import *
...
def test_list_lru(
    ...
    *,
    ...
    for mnt in for_each_mount(
        ...
        # be found by memcg
        print(f'memcg aware test on {mnt_dst} dentry lru at {hex(lru.address_of_())}')
        for dentry in list_lru_for_each_entry(prog, 'struct dentry',
           lru.address_of_(), 'd_lru') :
            d_cnt = d_cnt + 1
            ...
            if memcg == -1 :
                ...
                for n in for_each_online_node(prog) :
                    ...
                    if found == 0 :
                        if verbose is not None :
                            print(f'looking for dentry {hex(dentry)} in memcg {memcg} node {n}')
                        for dentry2 in list_lru_from_memcg_node_for_each_entry(prog,
                           memcg, n, 'struct dentry', lru.address_of_(), 'd_lru') :
                            if hex(dentry) == hex(dentry2) :
                                found = 1
                                break
                if found == 0 :
                    if verbose is not None :
                        print(f'lru for dentry {hex(dentry)} NOT found at mencg idx {memcg}')
                    errors = errors + 1
                else :
                    if verbose is not None :
                        print(f'lru for dentry {hex(dentry)} FOUND at mencg idx {memcg}')
        print(f'errors {errors} in {d_cnt} dentrys')
        if mnt_fstype == 'xfs' :
            print(f'memcg unaware test on {mnt_dst} xfs_buf lru at {hex(lru.address_of_())}')
            # memcg unaware lru
            ...
            for bp in list_lru_for_each_entry(prog, 'struct xfs_buf',
               lru.address_of_(), 'b_lru') :
                d_cnt = d_cnt + 1
                ...
                for n in for_each_online_node(prog) :
                    if verbose is not None :
                        print(f'looking for bmap {hex(bp)} in memcg {memcg} node {n}')
                    for bp2 in list_lru_from_memcg_node_for_each_entry(prog,
                       memcg, n, 'struct xfs_buf', lru.address_of_(), 'b_lru') :
                        if hex(bp) == hex(bp) :
                            found = 1
                            break
                if found == 0 :
                    if verbose is not None :
                         print(f'lru for bp {hex(bp)} NOT found at mencg idx {memcg}')
                    errors = errors + 1
                else :
                    if verbose is not None :
                         print(f'lru for bp {hex(bp)} FOUND at mencg idx {memcg}')
